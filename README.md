...
1. In the tutorial case, we are using the **Push model** of the Observer Pattern. The `NotificationService::notify` function actively retrieves subscriber information and directly sends (pushes) notifications to each subscriber's `update` method. This is evident in the `thread::spawn` call, where the notification payload is directly passed (pushed) to the subscriber's `update` method within a new thread.

2.  If we were to use the **Pull model**, subscribers would be responsible for requesting (pulling) the notification data from the publisher (e.g., the `NotificationService`). An advantage of the Pull model is that subscribers can selectively retrieve only the data they need, reducing the amount of data transferred and potentially improving efficiency. However, a significant disadvantage in this tutorial case is increased complexity. Subscribers would need to know the structure and location of the notification data and implement the logic to fetch it. This would add overhead and make the system more brittle, especially if the notification data structure changes. Additionally, the Push model is more suitable for real-time notifications, as it ensures immediate delivery, which is likely a requirement for the `bambangshop` application.

3. If we removed multi-threading from the notification process in `NotificationService::notify`, the program's performance would be significantly impacted, especially with a large number of subscribers. Instead of sending notifications concurrently, the program would send them sequentially, one after the other. This would lead to increased latency, as each subscriber's `update` method would have to complete before the next notification could be sent. For users, this would translate to a delay in receiving notifications, potentially degrading the user experience. Moreover, if any subscriber's `update` method encountered a delay or error, it could block the entire notification process, affecting all other subscribers. Multi-threading, in this scenario, is crucial to maintain responsiveness and prevent blocking, ensuring notifications are delivered promptly and independently.